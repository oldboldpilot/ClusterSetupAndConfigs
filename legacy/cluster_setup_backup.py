...
"""
    ...
    def fix_compiler_symlinks_and_path(self):
        """Fix GCC symlinks and verify all tools are on PATH after installation"""
        print("\n" + "="*60)
        print("POST-INSTALLATION: Fixing Symlinks and Verifying PATH")
        print("="*60)
        
        homebrew_bin = "/home/linuxbrew/.linuxbrew/bin"
        
        # 1. Fix GCC symlinks to point to latest version
        print("\n[1/3] Fixing GCC symlinks...")
        gcc_version_result = self.run_command(
            f"ls {homebrew_bin}/gcc-* 2>/dev/null | grep -E 'gcc-[0-9]+$' | sort -V | tail -1", 
            check=False
        )
        ...
        if gcc_version_result.returncode == 0 and gcc_version_result.stdout.strip():
            ...
            self.run_command(f"ln -sf gcc-{gcc_version} {homebrew_bin}/gcc", check=False)
            self.run_command(f"ln -sf g++-{gcc_version} {homebrew_bin}/g++", check=False)
            self.run_command(f"ln -sf gfortran-{gcc_version} {homebrew_bin}/gfortran", check=False)
            
            # Verify symlinks
            gcc_check = self.run_command(f"{homebrew_bin}/gcc --version", check=False)
            if gcc_check.returncode == 0:
                version_line = gcc_check.stdout.split('\n')[0]
                print(f"   ✓ GCC symlinks fixed: {version_line}")
            else:
                print(f"   ⚠️  GCC symlink verification failed")
        else:
            print("   ⚠️  Could not detect GCC version")
        
        # 2. Verify binutils is accessible
        print("\n[2/3] Verifying binutils...")
        binutils_path = "/home/linuxbrew/.linuxbrew/opt/binutils/bin"
        as_check = self.run_command(f"{binutils_path}/as --version", check=False)
        if as_check.returncode == 0:
            version_line = as_check.stdout.split('\n')[0]
            print(f"   ✓ Binutils accessible: {version_line}")
        else:
            print(f"   ⚠️  Binutils not found at {binutils_path}")
        
        # 3. Verify critical tools are on PATH
        print("\n[3/3] Verifying critical tools on PATH...")
        critical_tools = [
            ("gcc", f"{homebrew_bin}/gcc"),
            ("g++", f"{homebrew_bin}/g++"),
            ("gfortran", f"{homebrew_bin}/gfortran"),
            ("brew", f"{homebrew_bin}/brew"),
            ("python3", f"{homebrew_bin}/python3"),
            ("mpicc", f"{homebrew_bin}/mpicc"),
            ("mpirun", f"{homebrew_bin}/mpirun"),
        ]
        
        all_ok = True
        for tool_name, expected_path in critical_tools:
            which_result = self.run_command(f"which {tool_name}", check=False)
            if which_result.returncode == 0:
                actual_path = which_result.stdout.strip()
                if actual_path == expected_path or actual_path.startswith("/home/linuxbrew"):
                    print(f"   ✓ {tool_name}: {actual_path}")
                else:
                    print(f"   ⚠️  {tool_name}: {actual_path} (expected Homebrew)")
                    all_ok = False
            else:
        ...
        if all_ok:
            print("\n✅ All critical tools verified on PATH")
        else:
            print("\n⚠️  Some tools missing or not using Homebrew versions")
            print("   Run: eval \"$(/home/linuxbrew/.linuxbrew/bin/brew shellenv)\"")
    ...
    def run_full_setup(self, config_file: Optional[str] = None, non_interactive: bool = False):
        ...
        try:
            ...
            # Step 6: Post-Installation Verification (SIXTH - ensure everything is correct)
            self.fix_compiler_symlinks_and_path()
