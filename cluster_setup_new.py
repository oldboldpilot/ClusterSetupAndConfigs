...
class ClusterSetup:
    ...
    def _initialize_managers(self):
        ...
        self.ssh_mgr = SSHManager(self.username, self.password, 
                                  self.master_ip, self.worker_ips)
        self.sudo_mgr = SudoManager(self.username, self.password, 
                                    self.master_ip, self.worker_ips)
        
        # Network and infrastructure
        self.network_mgr = NetworkManager(self.username, self.password,
                                         self.master_ip, self.worker_ips)
        self.pdsh_mgr = PDSHManager(self.username, self.password,
                                   self.master_ip, self.worker_ips)
        
        # Parallel programming managers
        self.mpi_mgr = MPIManager(self.username, self.password,
                                 self.master_ip, self.worker_ips)
        self.openmp_mgr = OpenMPManager(self.username, self.password,
                                       self.master_ip, self.worker_ips)
        self.openshmem_mgr = OpenSHMEMManager(self.username, self.password,
                                             self.master_ip, self.worker_ips)
        self.berkeley_upc_mgr = BerkeleyUPCManager(self.username, self.password,
                                                  self.master_ip, self.worker_ips)
        
        # Workload and benchmark managers
        self.slurm_mgr = SlurmManager(self.username, self.password,
                                     self.master_ip, self.worker_ips)
        self.benchmark_mgr = BenchmarkManager(self.username, self.password,
                                             self.master_ip, self.worker_ips)
    ...
    def run_full_setup(self, config_file: Optional[str] = None, 
        ...
        try:
            # STEP 1: SSH Keys and Passwordless Sudo (FIRST - enables remote operations)
            print(f"\n{'='*70}")
            ...
            print(f"{'='*70}")
            self._install_upcxx_and_pgas_legacy()  # Will be migrated to manager
    ...
    def _install_openmpi_legacy(self):
        """Legacy OpenMPI installation - to be migrated to MPIManager"""
        print("\n=== Installing OpenMPI ===")
        # For now, call the legacy install_openmpi method
        # This will be fully migrated to MPIManager in the future
        from cluster_setup_monolithic import ClusterSetup as LegacySetup
        legacy = LegacySetup(self.master_ip, self.worker_ips, self.username, self.password)
        legacy.install_openmpi()
    
    def _install_upcxx_and_pgas_legacy(self):
        """Legacy PGAS installation - to be migrated to managers"""
        print("\n=== Installing UPC++ and PGAS Libraries ===")
        # For now, call the legacy method
        # This will be fully migrated to individual managers
        from cluster_setup_monolithic import ClusterSetup as LegacySetup
        legacy = LegacySetup(self.master_ip, self.worker_ips, self.username, self.password)
        legacy.install_upcxx_and_pgas()
    ...
    def _setup_other_nodes(self, config_file: str):
        ...
        # Import legacy setup for worker node setup
        from cluster_setup_monolithic import ClusterSetup as LegacySetup
        legacy = LegacySetup(self.master_ip, self.worker_ips, self.username, self.password)
        ...
        if other_nodes:
            legacy.setup_all_workers(config_file, other_nodes)
